...
function displayAllGuests() {
    return new Promise((resolve, reject) => {
        const query = `
            SELECT g.id, g.uuid, g.fullName, g.gender, g.respStatus, g.respDate, 
                   u.username, u.husbands_name, u.wifes_name
            FROM guest g 
        ...
        db.all(query, [], (err, rows) => {
            if (err) {
                ...
                console.log('ID | UUID (short) | Full Name           | Gender | Status | Response Date | User        | Couple');
                console.log('─'.repeat(140));
